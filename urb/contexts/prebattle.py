...
"""
    ...
    def com_pick(_self, self, args):
        ...
        if args['selector'].finalized == 0:
            self.session.msg("'%s' is not a valid character selector." % args['selector'].selector)
        else:
            self.app.game.player_pick(self.player, args['selector'].selector)
            self.session.msg("Make sure to 'ready' up if you're prepared for battle.")
    ...
    def com_moves(_self, self, args):
        ...
        if thechar:
            ...
            self.session.msg("You haven't selected a character yet.")
