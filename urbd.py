#!/usr/bin/env python
"""
URB SERVER STARTUP SCRIPT

This is the start point for running URB. 
...
This program launches URB with various options. You can access all
this functionality directly from the command line; for example option
five (restart server) would be "URB.py restart server".  Use
"urbd.py -h" for command line options.

URB consists of two separate programs that both must be running
...
if not os.path.exists('settings.py'):
    ...
    URB as desired by copy&pasting options from
    ...
    urb.db3 appearing. This is the database file. Just delete this
    and repeat the above manage.py steps to start with a fresh
    database.

    When you are set up, run urbd.py again to start the server.""")
    sys.exit()
